                            ...
                            Expanded(
                              child: TextField(
                                onChanged: (value) {
                                  setState(() {
                                    if (value.isNotEmpty) {
                                      localContactCtrl.searchData = [];
                                      for (int i = 0;
                                          i <
                                              localContactCtrl
                                                  .contactDetails.length;
                                          i++) {
                                        String name = localContactCtrl
                                            .contactDetails[i]["name"];
                                        String number = localContactCtrl
                                            .contactDetails[i]["number"];
                                        if (name.toLowerCase().contains(
                                                value.toLowerCase()) ||
                                            number.contains(value)) {
                                          localContactCtrl.searchData.add(
                                              localContactCtrl
                                                  .contactDetails[i]);
                                          print(
                                              "Search===${localContactCtrl.searchData}");
                                        }
                                      }
                                    } else {
                                      localContactCtrl.searchData =
                                          localContactCtrl.contactDetails;
                                    }
                                  });
                                },
                                autofocus: true,
                                controller: localContactCtrl.searchController,
                  ...
                  return BackdropFilter(
                    ...
                    child: AlertDialog(
                      title: const Column(children: [
                        Text("contactdetail"),
                        SizedBox(height: 20),
                        Tooltip(message: "addimg", child: Icon(Icons.contacts))
                      ]),
                      actions: [
                        TextField(
                        ...
                        GestureDetector(
                            onTap: () {
                              int user =
                                  SplashScreen.prefs!.getInt("userid") ?? 0;
                              DataBaseHelper()
                                  .insertContact(localContactCtrl.name.text,
                                      localContactCtrl.number.text, user, db!)
                                  .then((value) {
                                localContactCtrl.viewData(context);
                              });
